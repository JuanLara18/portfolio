...
A ticket in the platform team's queue: provision a managed Postgres instance, small tier, in the claims project. Standard stuff. The template was filled correctly. The cost centre was valid. The justification field was more coherent than most. The only unusual thing was the requester, which was not a person. It read `agent://claims/document-triage`, and it had opened the ticket because an insurer had dumped forty thousand scanned PDFs into an intake bucket and the agent had reasoned, correctly, that it needed somewhere structured to put the extracted fields.

Nobody had designed for this. The provisioning template assumed a human would read the confirmation email. The approval step assumed a manager would recognise the requester's name. The quota model assumed requests arrived at human frequency, four or five a quarter per team, not at whatever frequency an autonomous system decides is warranted. And the audit trail, the part that actually mattered, recorded a service account that eleven different agents shared, because when the identity was created six months earlier nobody had imagined there would be eleven.

The platform team's first instinct was to block it. Their second, which was correct, was to notice that the agent had done exactly the right thing: it wanted a database, so it asked the platform for a database, through the front door, with a justification and a cost centre. That is the behaviour you spend years trying to teach human engineers. What was missing was not the agent's discipline. It was the platform's.

For roughly a decade, platform engineering has had one clear mandate: make it easy for developers to do the right thing. That mandate produced golden paths, paved roads, internal developer platforms, and the whole vocabulary of shifting security left so it arrives as a default rather than a rejection. It worked. It made compliance a side effect of convenience rather than a tax on velocity.

That mandate has just been handed a much larger scope. The organisation now wants agents everywhere: in claims, in finance, in the lakehouse, in the service desk, in the code review pipeline. Every one of those agents will need compute, credentials, data, tools, budget, observability, and a way to be turned off. Every one will be built by a team that has never built one before. And every one will, if left alone, invent its own answer to all of those questions.

The teams that built golden paths for developer autonomy are now being handed the keys to enterprise-wide agentic autonomy. This is the largest mandate platform engineering has ever held, and it arrived with almost no warning. The [PlatformCon 2026 keynote](https://platformengineering.org/blog/platformcon-2026-wrap-up-no-ai-at-scale-without-platform-engineering) put it bluntly: there is no usage of AI in the enterprise at scale without platform engineering. I would put it more sharply. Every organisation is going to get an agent platform. The only question is whether it is one platform designed on purpose, or forty platforms that emerged by accident and share nothing but a cloud bill.

This is the fifth and final part of *The Agent Platform*. The first four described components. [Part 1](https://juanlara18.github.io/portfolio/#/blog/agent-platform-control-plane-data-plane) split the world into a control plane and a data plane. [Part 2](https://juanlara18.github.io/portfolio/#/blog/agent-runtime-sessions-state-topology) built the runtime, with its sessions, its state, and its execution topology. [Part 3](https://juanlara18.github.io/portfolio/#/blog/sandboxing-agents-microvm-gvisor) put the untrusted parts inside microVMs and gVisor. [Part 4](https://juanlara18.github.io/portfolio/#/blog/mcp-registry-gateway-tool-plane) built the tool plane out of an MCP registry and a gateway.

Those are components. A pile of components is not a platform. What turns them into one is the subject of this post: the product discipline, the tenancy model, the policy enforcement, and the paved road that makes all of it the easiest thing a team can do.
...
Platform engineering's foundational ideas were not artefacts of the container era. They are answers to a durable organisational problem: how do you let many autonomous teams move fast without each of them independently solving networking, identity, deployment, and compliance badly? The answers were, and remain:

**Platform as product.** The platform has users, and those users can leave. It has a roadmap, a backlog, and a support burden. It is staffed with someone who owns the user experience, not just the SLOs. Skelton and Pais built this into *Team Topologies* as the defining property of a platform team: the platform exists to reduce the cognitive load of the stream-aligned teams it serves, and its success is measured by their flow, not by its own feature count.

**Golden paths.** Spotify's [original formulation](https://engineering.atspotify.com/2020/08/how-we-use-golden-paths-to-solve-fragmentation-in-our-software-ecosystem) is still the best one: the golden path is "the opinionated and supported path to build something." Opinionated, so there is a default. Supported, so choosing the default buys you something concrete. Both halves matter, and the second half is the one organisations forget.
...
**Self-service with guardrails.** The platform's job is to make the safe action the fast action, so that nobody has to choose between shipping and complying.

What changes is not the principles. What changes is **who the consumers are**, and that turns out to change a great deal downstream. The [Platform Engineering 2.0 framing](https://www.cncf.io/blog/2026/07/06/evolving-platform-engineering-for-ai-native-workloads/) makes the multi-persona point explicitly: a platform now serves developers, data scientists, ML engineers, FinOps practitioners, security teams, and AI agents themselves, each with distinct self-service needs and distinct governance requirements. The same source names the other structural shifts: first-class GPU and TPU allocation, model serving, MCP gateways, agentic guardrails, and cost intelligence that moves "from bolt-on reporting to provisioning-time decisioning."

That last phrase is a good example of the pattern. Cost reporting was always a platform concern; what changes when your consumers are agents is that reporting after the fact is too slow, because the thing that spent the money was not a human who can be asked about it next Tuesday. The principle is unchanged, the latency requirement is completely different. Hold that shape for the rest of this post: stable principles, new consumers, large consequences.
...
Agents are a genuinely new persona, and they are new in ways that break specific assumptions rather than vague ones. Five of them matter.

**An agent has no eyes.** Every interface a platform team is proud of, the portal, the wizard, the diagram, the well-designed error page, is invisible to an agent. The [Platform Engineering 2.0 material](https://platformengineering.org/reports/platform-engineering-2-0-an-evolution-for-the-ai-era) states the requirement precisely: agents need the same golden paths, the same role-based access control, and the same audit trail as developers, "just exposed through machine interfaces like MCP, not a user interface." A portal-only platform is, from an agent's perspective, no platform at all, and an agent that cannot use your platform will use `curl` and a long-lived API key instead.

**An agent has no employment record.** When a human joins, an identity is created by a process that predates the platform, is owned by HR and IAM, and expires when they leave. Agents have none of that. Their identity must be minted, scoped, attested, and expired by the platform itself, which is usually the only system that knows the agent exists. This is the argument for workload identity: cryptographically attested, short-lived credentials issued at startup rather than static keys pasted into config. SPIFFE and SPIRE are the mature answer in the cloud native ecosystem, and the emerging consensus is that agent identity is workload identity plus delegated authorization, not a new invention. The practical implication is simpler than the standards debate: **the identity must be created by the scaffold, not requested afterwards**, because every identity requested afterwards is created in a hurry by someone who needs it working today.

**An agent operates at machine frequency.** A team of six engineers might file twenty provisioning requests a year. A fleet of agents can file twenty in an afternoon, and unlike the engineers, they will not get discouraged and give up. Any step in the golden path with a synchronous human approver becomes a queue, then a bottleneck, then a bypass. The consequence is not to remove human judgement but to relocate it: humans set policy in advance, machines evaluate it per request, and human review is reserved for the exceptions policy explicitly routes to them. Approval becomes a rule someone wrote last quarter, not a message someone reads at 4pm.

**An agent is almost always acting for someone else.** This is the subtlest one and the source of the worst incidents. A human developer acts on their own behalf; their identity is the whole story. An agent acts on behalf of a user, a team, a scheduled job, or another agent. So there are always at least two identities in play: the agent's own workload identity, which says what code is running, and the delegated principal, which says whose authority it is exercising. Collapsing these is how you get an agent that is technically well-scoped and still answers a contractor's question using a document only the board can see. The question is never "can this agent read the HR corpus." It is "can this agent read the HR corpus **on behalf of this particular person, right now**." We will return to this in the tenancy section, because it is the hinge on which the whole data isolation problem turns.

**An agent does not learn from a bad experience.** A human who files a malformed request gets an error, mutters, and fixes it. An agent that files a malformed request will file the identical request again, and again, at whatever rate its retry policy allows, until something upstream changes. Deny-by-default and hard quotas are not paranoia in this world; they are the only things standing between a misclassified error and a very expensive night.

### The design rule that follows

All five points converge on one rule, and it is the most important sentence in this post.
...
The CNCF's [agentic enterprise](https://www.cncf.io/blog/2026/07/21/platform-engineering-for-the-agentic-enterprise-managing-applications-resources-and-ai-agents/) framing lands in the same place: applications, resources, and agents all become first-class managed platform objects, and each actor, human or agent, gets "a distinct identity, scoped permissions, and a clear audit trail." The reference implementation they point at, OpenChoreo, exposes the same capabilities through portals, CLI, APIs, and GitOps for humans and through MCP servers for agents, deliberately keeping one governance model behind two interfaces.

The failure mode is the alternative, and it is seductive because it is faster in week one. You build a small side channel so the agent team can get moving: a service account with broad permissions, a direct Terraform path, a Slack bot that provisions things. Six months later that side channel carries more traffic than the front door, has none of the policy evaluation, and is invisible to every dashboard the platform team owns. **A second provisioning path for agents is not a shortcut, it is a shadow platform**, and it will be the one that has the incident.
...
Two details in that diagram are load-bearing. The agent receives a **credential handle**, not a secret, so the secret never enters a context window or a trace. And the denial path returns **a remediation the agent can act on**, because an error message written for a human to read is a dead end for a non-human consumer. Machine-readable failures are an accessibility requirement for this persona.

The full persona map looks like this, and it is worth drawing for your own organisation, because the second column is where most platforms discover a gap.
...
Abstraction is comfortable, so let us be concrete. A team gets funding to build a claims triage agent. They have built services before, never an agent. They open the portal, pick "new agent," and answer four questions: what is it for, which tenant owns it, what data classification does it touch, and who is accountable for it.

Ninety seconds later, seven things exist that did not exist before.

**1. A scaffolded repository.** Not an empty repo with a README. A repo containing an agent manifest, a working hello-world agent that already returns an answer, an eval suite with three seeded cases and a harness that runs them, a CI pipeline with the policy, eval, and cost gates already wired, a Dockerfile matched to the platform's runtime contract, and a runbook stub with the kill switch procedure already filled in. The scaffold is not documentation about how to do things correctly. It is a repository that is already doing them.

**2. A pre-wired runtime.** The session store, checkpointing, and execution topology from [Part 2](https://juanlara18.github.io/portfolio/#/blog/agent-runtime-sessions-state-topology) are chosen and configured, with the sandbox profile from [Part 3](https://juanlara18.github.io/portfolio/#/blog/sandboxing-agents-microvm-gvisor) applied by default. The team does not choose an isolation model. They inherit a good one and can request a different one through an exception if they genuinely need it. Very few will.

**3. A registered non-human identity with least-privilege defaults.** Minted at scaffold time, scoped to the tenant, with zero standing privilege beyond reading its own configuration. Every additional permission is an explicit, reviewable diff against a baseline of nothing. This ordering matters enormously: when the identity is created first and permissions are added, the diff is legible. When permissions are created first and scoped later, scoping never happens.

**4. A curated tool catalog subset.** The MCP registry from [Part 4](https://juanlara18.github.io/portfolio/#/blog/mcp-registry-gateway-tool-plane) contains hundreds of tools. This agent is bound to eleven of them, selected by the data classification it declared. The gateway enforces the binding; the agent cannot reach a twelfth tool by guessing its name. Curation is the cheapest guardrail on the entire platform, and it happens at scaffold time for free.

**5. Observability and an eval harness wired from day one.** Traces flow to the platform's collector before the team writes their first prompt. The eval suite runs on every commit. This is the difference between a team that instruments their agent because a post-incident review demanded it and a team for whom instrumentation was simply how the repository arrived.

**6. Cost attribution tagged automatically.** Tenant, agent, environment, and cost centre labels are minted with the identity and propagated by the gateway onto every model call. Nobody remembers to add a tag, because nobody adds tags. This is the structural version of the metering discipline in [FinOps for AI Agents](https://juanlara18.github.io/portfolio/#/blog/finops-llm-agents-token-economics), and its whole value is that it is not voluntary.
...
All seven collapse into a single declarative artefact. This is the file the scaffold writes and the file every downstream control reads.
...
The point of drawing it as a journey rather than an architecture is that **the golden path's product is elapsed time**, not capability. A platform that can theoretically do all of this in six weeks of tickets has not built a golden path; it has built a catalogue of services with a long form in front of it. The thing you are actually selling is that a team with no agent experience reaches a governed sandbox before lunch.

The way you get there is counter-intuitive to most platform teams: **make the defaults correct rather than making the options rich**. Every configurable knob is a decision you have handed back to a team that does not want it and is not qualified to make it. Team Topologies calls the target a thinnest viable platform, and the discipline it demands is the discipline of deleting options.

One more observation, because it is the highest-leverage thing in this section. **The scaffold is the enforcement point.** Anything present at scaffold time is simply how things are; nobody argues with it, nobody removes it, and nobody experiences it as a control. Anything you introduce later is a negotiation with a team that has a deadline. Security that arrives in the template is free. The identical security arriving in a review is a conflict. Put everything you care about in the template.
...
The easy layer. Every tenant, meaning a team or a business line, gets a namespace: its own agents, its own environments, its own identity prefix, its own quota pool, its own cost centre. This is standard multi-tenant platform work and the cloud native ecosystem has good answers. The agent-specific twist is that the quota unit is unfamiliar. You are not primarily rationing CPU. You are rationing **tokens per minute, concurrent sessions, tool calls per minute, and dollars per month**, and those four are only loosely correlated with each other. A tenant can be well within its dollar budget while saturating your entire provider throughput allocation with cheap calls.
...
Secrets are where a tenancy model quietly fails. The pattern to avoid is a platform-wide secret store where any agent identity can read any path, protected only by the fact that agents do not usually guess paths. Secrets should be scoped to the tenant namespace, injected into the runtime rather than into the context window, and short-lived by default. The agent receives a handle it can present to the gateway; the gateway resolves it. An agent that never holds a raw credential cannot leak one through a trace, a log line, a summary, or a prompt-injected exfiltration.

### Data isolation on a shared knowledge base

Now the dangerous one. This is where the worst incidents in enterprise agent programs happen, and they follow a consistent shape: multiple tenants share one vector store or one knowledge base, isolation is enforced somewhere other than the query, and one day a retrieval returns a document to a principal who should never have seen it.

**Start with the pattern choice.** There are three, and the tradeoffs are well understood in the vector database literature. Pinecone's [multi-tenancy guidance](https://www.pinecone.io/learn/series/vector-databases-in-production-for-busy-engineers/vector-database-multi-tenancy/) is representative and pragmatic: prefer **one namespace per tenant** unless you specifically need to query across tenants. Namespaces physically partition records, so a query touches one tenant's data and no other; you get performance isolation, so one tenant's burst does not slow another's queries; and offboarding becomes a single delete rather than a filtered sweep. A **shared index with metadata filtering** keeps cross-tenant querying possible but shares compute and storage, makes per-tenant cost attribution difficult, and means a single filter bug is a full cross-tenant read. **An index or cluster per tenant** gives the strongest isolation and the highest operational cost, and is what regulated tiers usually end up buying for their most sensitive corpora.

The clean synthesis: **namespaces for tenant separation, metadata filters for sub-tenant document-level access control.** The two mechanisms answer different questions and you need both.
...
**Now the part that actually matters: enforce at retrieval, never at prompt.** The single most common enterprise RAG vulnerability is a system prompt containing a sentence like "only use documents belonging to the current user's tenant." That is not access control. That is a polite request to a probabilistic system, and it is trivially defeated by an indirect prompt injection sitting inside one of the documents you retrieved. The industry vocabulary for the correct approach is ACL-aware retrieval: access control enforced as a filter **applied during the nearest-neighbour query itself**, before any content reaches the model. Retrieve-then-check is better than nothing but still leaks through result counts, latency, and citation metadata. Check-then-retrieve is the only version that holds.

Three refinements that separate a design that works from one that mostly works:

**Chunk-level, not document-level, permissions.** A document with mixed sensitivity gets chunked, and if every chunk inherits the document's top-level ACL, the confidential appendix becomes retrievable by anyone allowed to see the public summary. Permissions must be evaluated per chunk, which means your ingestion pipeline has to preserve section-level access metadata rather than flattening it.

**Pre-filter, not post-filter.** For large corpora with a low positive hit rate for a given principal, post-filtering forces the index to over-fetch enormously and still risks returning nothing useful. Pre-filtering inside the ANN query is both faster and safer.

**Derived artefacts inherit nothing.** This is the leak nobody audits. Your vector store has an ACL model. Your **trace store, your response cache, your agent memory, your summarisation checkpoints, and your eval datasets do not**, and every one of them is a copy of tenant data. I have seen an organisation with an immaculate retrieval ACL model whose observability backend let any platform engineer read the full text of every retrieved passage across every tenant, because traces were treated as telemetry rather than as data. Every derived store is a tenancy boundary. Label it, scope it, and expire it.

Here is the enforcement in code. The design goal is that it is **impossible to issue an unscoped query**, not merely discouraged.
...
I have covered the mechanics of token metering, budgets, and unit economics at length in [FinOps for AI Agents](https://juanlara18.github.io/portfolio/#/blog/finops-llm-agents-token-economics), so I will not re-derive the cost equation here. What belongs in *this* post is the platform-political argument, which is different and which platform teams consistently underweight.

The requirement is simple to state: **every token traceable to a tenant, an agent, and ideally a business outcome.** Tenant tells you whose budget it is. Agent tells you which workload to fix. Business outcome, meaning the resolved ticket or processed claim, tells you whether the spend was worth anything. Most platforms achieve the first, some achieve the second, almost none achieve the third, and the third is the one that wins budget arguments.
...
**Attribution must be structural, not voluntary.** A tag that developers are asked to add is a tag that is present on sixty percent of traffic, which is worse than useless because the gap is invisible and non-random. Labels minted with the identity at scaffold time and stamped by the gateway on every call are present on one hundred percent, because there is no code path that omits them. This is the same argument as putting security in the scaffold, applied to money.

**Cost belongs at provisioning time, not month end.** The Platform Engineering 2.0 framing calls this moving cost intelligence "from bolt-on reporting to provisioning-time decisioning," and for agents it is not optional. Showing a team the price of the flagship model *in the scaffold*, when they are choosing, changes the choice. Showing them the same number in a report on the third of next month changes nothing, because the decision is three weeks cold and the code already works.

Showback first, chargeback when the organisation is ready. Behaviour changes on showback alone, and it changes fast, because engineers are not indifferent to spend; they are usually just unaware of it.

---

## Policy as Code: Admission Control for Agents

Guardrails that live in a wiki are guardrails that exist only for the people who read the wiki, which at 6pm on a Friday is nobody. The platform's job is to convert intent into enforcement, and the mechanism is policy as code: rules written in a versioned repository, reviewed like code, evaluated by an engine, and applied identically to every request without anyone remembering to apply them.

The design question that actually matters is **what gets blocked at deploy time versus at runtime**, and there is a clean rule for it.
...
Deploy-time denial is cheap: it happens in CI, to an engineer, in the context of a change they are actively making, with a fix they can apply in the next commit. Runtime denial is expensive: it happens to a user, in production, as a failure. So push everything you can decide statically into admission control, and reserve runtime enforcement for what genuinely cannot be known until the request arrives.
...
Two things about that policy deserve emphasis. The `enforce-at-retrieval` rule is the one I would keep if I could keep only one, because it is the difference between a data isolation model and a data isolation aspiration. And the escape hatch is *in the policy file*, which is the whole argument of the next section: the exception is a first-class, expiring, owned, compensated object, not a thing that happens by someone quietly not using the platform.

---

## The Paved Road Versus the Walled Garden

Everything above can be built correctly and still fail. This is the failure mode platform engineering has been rediscovering for fifteen years, and agents will not exempt anyone from it.

The failure is this. The platform team, handed a governance mandate and a genuine fear of what an ungoverned agent might do, makes the golden path **mandatory** and then makes it **narrow**. Only approved models. Only registry tools. Only these two topologies. Exceptions by committee, monthly. The intent is reasonable. The result is that the path stops being a path and becomes a fence, and engineers do to fences what water does to a dam: they find the lowest point.

What happens next is worse than what the fence was built to prevent. A team with a deadline provisions a project in a corner of the cloud estate, gets a personal API key, builds a genuinely useful agent, and connects it to production data. No identity model, no eval suite, no cost attribution, no kill switch, not in any inventory. That is shadow AI, and it is not a discipline problem. It is a **product failure by the platform team**, and it should be diagnosed as one.
...
**Some things are genuinely mandatory, and the list must be short and defensible.** Identity, data isolation, kill switch, cost attribution, inventory registration. Five things, each of which you can explain to an engineer in one sentence about what goes wrong without it. If the mandatory list has thirty items, nobody knows which five matter and the whole list gets treated as bureaucracy.

**Everything else is a default, not a requirement.** Model choice, topology, memory strategy, evaluation depth. These are opinions with defaults. Teams that want to deviate should be able to, and the platform should be measurably more convenient for teams that do not.

**The escape hatch is documented, owned, and expiring.** That is what the exception block in the Rego policy is for. An exception has an approver, a compensating control, and an expiry date. It is a registered object, not an absence of one. The exception register is also, and this is the useful part, your product backlog: when six teams register the same exception, you have not found six rule-breakers, you have found a missing golden path.

**Off-path usage is a product signal, not a compliance finding.** The instinct when a team goes around the platform is to escalate. The productive response is to ask what they needed that the path did not provide, and to treat the answer as a requirement. Every off-path agent is a user research interview you did not have to schedule.
...
- Skelton, M. & Pais, M. (2019). *Team Topologies: Organizing Business and Technology Teams for Fast Flow.* IT Revolution Press.
  - The origin of platform-as-product as a formal idea, plus team cognitive load and the thinnest viable platform. Every argument here about deleting options and measuring consumer flow traces back to it.
...
- Hohpe, G. (2020). *The Software Architect Elevator.* O'Reilly Media.
  - On carrying technical decisions up and down an organisation, which is most of what an agent platform lead does. The control-versus-autonomy chapters are the walled garden problem in general form.
- Kleppmann, M. (2017). *Designing Data-Intensive Applications.* O'Reilly Media.
  - The partitioning and derived-data chapters sit under the shared knowledge base section, including the point that every derived store is its own consistency and access problem.

**Online Resources:**

- [Evolving platform engineering for AI-native workloads](https://www.cncf.io/blog/2026/07/06/evolving-platform-engineering-for-ai-native-workloads/) — CNCF's framing of Platform Engineering 2.0: AI-native support, multi-persona experience, embedded FinOps, security integration, and composable architecture. The source of the multi-persona argument in this post.
- [Platform engineering for the agentic enterprise](https://www.cncf.io/blog/2026/07/21/platform-engineering-for-the-agentic-enterprise-managing-applications-resources-and-ai-agents/) — Applications, resources, and agents as first-class managed platform objects, with one governance model behind human and machine interfaces. Includes the OpenChoreo reference implementation.
- [How we use Golden Paths to solve fragmentation in our software ecosystem](https://engineering.atspotify.com/2020/08/how-we-use-golden-paths-to-solve-fragmentation-in-our-software-ecosystem) — Spotify's original post. Short, and still the best statement of "opinionated and supported" with a deliberate escape hatch.
- [CNCF Platform Engineering Maturity Model](https://tag-app-delivery.cncf.io/whitepapers/platform-eng-maturity-model/) — Investment, Adoption, Interfaces, Operations, and Measurement, with the explicit warning that maximum maturity is not the goal. A good self-assessment before you scale an agent platform.
- [Multi-Tenancy in Vector Databases](https://www.pinecone.io/learn/series/vector-databases-in-production-for-busy-engineers/vector-database-multi-tenancy/) — Namespace per tenant versus metadata filtering in a shared index, with the performance isolation, cost, and offboarding tradeoffs spelled out.
- [PlatformCon 2026 wrap-up](https://platformengineering.org/blog/platformcon-2026-wrap-up-no-ai-at-scale-without-platform-engineering) — The agentic development platform framing, context as code, and the recurring theme that AI amplifies existing organisational dysfunction rather than removing it.

**Videos:**

- [Webinar: Platform as a Product - Latest Insights & Updates from Manuel Pais & Matthew Skelton](https://www.youtube.com/watch?v=DKeAIpBciq0) by Team Topologies — The authors on what platform-as-product means in practice, including thinnest viable platform and team interaction modes. The clearest statement of the discipline this post extends to agents.
- [Matthew Skelton and Manuel Pais on Team Topologies](https://www.youtube.com/watch?v=XhPm0iNLpdU) by InfoQ — A shorter interview covering cognitive load, team types, and why platform teams fail when they optimise for their own architecture rather than their consumers' flow.
...
- Jin, Y., Wu, Y., Hu, W., Maggs, B. M., Zhang, X., & Zhuo, D. (2024). ["Curator: Efficient Indexing for Multi-Tenant Vector Databases."](https://arxiv.org/abs/2401.07119) *arXiv:2401.07119.*
  - The systems research behind the tenancy tradeoff in the isolation section: tenant-specific clustering trees encoded as sub-trees of a shared structure, chasing low memory overhead and strong per-tenant query performance simultaneously. Read it before deciding that per-tenant indexes are the only safe option.
- Forsgren, N., Storey, M.-A., Maddila, C., Zimmermann, T., Houck, B., & Butler, J. (2021). ["The SPACE of Developer Productivity: There's more to it than you think."](https://dl.acm.org/doi/10.1145/3454122.3454124) *ACM Queue, 19(1).*
  - Satisfaction, Performance, Activity, Communication, and Efficiency. The methodological warning underneath the metrics table: productivity is multi-dimensional, and any single number you promote to a target will be gamed.
- Greshake, K., Abdelnabi, S., Mishra, S., Endres, C., Holz, T., & Fritz, M. (2023). ["Not What You've Signed Up For: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection."](https://arxiv.org/abs/2302.12173) *arXiv:2302.12173.*
  - The formal reason prompt-level access control is not access control. If retrieved content can rewrite the model's instructions, then any isolation rule stated in the prompt is advisory.
